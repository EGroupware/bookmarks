 ...
 * @package bookmarks
 * @author Hadi Nategh	<hn-AT-egrouwpare.org>
...
class BookmarksApp extends EgwApp
{
	...
	tree_action(_action, _selected)
	{
		...
		switch (_action.id)
		{
			case 'visit':
				...
				break;
			case 'edit':
				this.egw.open_link(this.egw.link('/index.php','menuaction=bookmarks.bookmarks_ui.edit&bm_id='+id),'',
					<string>this.egw.link_get_registry('bookmarks','add_popup'), 'bookmarks');
				break;
			case 'add':
				this.egw.openPopup(this.egw.link('/index.php','menuaction=bookmarks.bookmarks_ui.create&cat_id='+id),
					750,300,'_blank');
				break;
			case 'mailto':
				var selected = [];
	 ...
	 */
	setState(state, template? :string) : string|false
	{
